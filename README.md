...
三个模块以前各自写死了自己的 BEV 分辨率（OSZ 用 0.4m、PA_gen_v1 用 0.5m、
PA_gen_v2 用 0.2m），格子对不齐，任何"把 OSZ 结果喂给 Phantom Agent 训练"的用法都会
...
只是覆盖之后这一次跑的网格就不再和 PA_gen_v1/PA_gen_v2 对齐了，除非你也
同步改了它们的分辨率。
...
- **PA_gen_v2/**：和 preprocess/ 目标类似（挖 Phantom Agent 正/负样本），但用
  ...
  修复方案，见 [`PA_gen_v2/README.md`](PA_gen_v2/README.md)。
