...
```
common/       项目唯一的 BEV 网格配置来源（改一个数字，OSZ/preprocess/filter 全部联动）
OSZ/          相机深度图 → 3D 体素投射 → 2D BEV 射线投射，得到遮挡阴影区
preprocess/   基于 nuScenes 标注 visibility_token 挖 Phantom Agent 训练标签
filter/       基于几何遮挡（复用 OSZ/ 的计算）挖"幽灵车辆出现事件"，方法与用途见 filter/README.md
```
...
`OSZ/run_osz_pipeline.py`、`preprocess/create_pa_labels_mini.py`、
`preprocess/create_pa_labels_full.py`、`filter/osz_source.py` 都从这里取值。
...
`preprocess/create_bevdet_pkl_mini.py`、`create_bevdet_pkl_full.py`、
`preprocess/pa_visible.py` 不涉及 BEV 网格化（只产出原始 3D 框数据，或者是纯
标注可视化），不需要跟着这个配置改动。
...
- **preprocess/**：直接读 nuScenes 标注自带的 `visibility_token`，找"过去被
  强遮挡、之后突然变清晰可见"的车辆标注链，生成 Phantom Agent 的 BEV 热力图
  训练标签。不依赖任何自己算的遮挡几何。

- **filter/**：和 preprocess/ 目标类似（挖 Phantom Agent 正/负样本），但用
  OSZ/ 算出来的真实几何遮挡区去判断，而不是依赖标注自带的可见度字段。
  现在 `filter/` 的 OSZ 计算直接调用 `OSZ/modules/ray_casting.py`（通过
  `filter/osz_source.py` 这层薄适配），不再维护自己的第二套实现。
  详细说明、坐标轴约定的坑、以及"回溯帧没标注就默认判定被遮挡"这个隐藏假设的
  修复方案，见 [`filter/README.md`](filter/README.md)。
