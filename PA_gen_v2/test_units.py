...
# ── Test: drivable-filter graceful degradation ───────────────────────────
def test_drivable_mask_fallback_shape_and_dtype():
    """
    get_drivable_mask_for_sample() must always return a (nx, ny) bool
    array, whether or not shapely/the map API are importable. When they
    are NOT importable, this must fall back to an all-True mask WITHOUT
    ever touching the `nusc` argument — that's what lets this test run
    with nusc=None regardless of environment, and it's the actual
    contract callers rely on (get_pa_relevant_osz_for_sample() must not
    crash just because the optional map dependency is missing).
    """
    osz_source.clear_cache()
    mask = osz_source.get_drivable_mask_for_sample(None, '__test_token__')
    nx, ny = osz_source.grid_shape()
    assert mask.shape == (nx, ny)
    assert mask.dtype == bool
    if not osz_source.drivable_filter_available():
        assert mask.all(), \
            "with drivable filtering unavailable, fallback must be all-True (no filtering)"
    osz_source.clear_cache()
...
if __name__ == '__main__':
    tests = [
        ...
        ("drivable-filter graceful degradation",  test_drivable_mask_fallback_shape_and_dtype),
