...
1. OSZ now comes from OSZ/modules/ray_casting.py via osz_source.py,
   ...
   used to live in this file's import (filter/osz_geometry.py, now kept
   only as filter/osz_geometry_legacy.py for reference — nothing in the
   active pipeline imports it).
2. The "unannotated lookback frame = assume occluded" shortcut is gone.
   See filter/trajectory.py's module docstring for the full rationale;
...
def mine_ghost_events(
    ...
    for scene_token in tqdm(scene_tokens, desc='Scenes'):
        ...
        for sample_token in sample_tokens_in_scene:
            ...
            try:
                bev_occ_t, osz_mask_t = osz_source.get_osz_for_sample(nusc, sample_token)
            ...
            for lb_tok in lookback_tokens:
                try:
                    _, osz_lb = osz_source.get_osz_for_sample(nusc, lb_tok)
            ...
            for ann_t in anns_t:
                ...
                if osz_source.is_in_osz(pt_ego_t[0], pt_ego_t[1], osz_mask_t):
                ...
                for lb_idx, lb_xyz_global in enumerate(lb_positions):
                    ...
                    if lb_xyz_global is not None:
                        ...
                        was_in_osz_per_frame.append(
                            osz_source.is_in_osz(pt_ego_lb[0], pt_ego_lb[1], osz_lb))
                        continue
