...
BEV 网格：
  ...
  网格定义源），不再在本文件里单独写死。要改整个项目的 BEV 格子大小，
  去改 common/bev_config.py 里的 BEV_RESOLUTION_M 一个数字，本文件、
  create_pa_labels_mini.py、OSZ/run_osz_pipeline.py、filter/osz_source.py
  会同时生效，格子仍然保证对齐。
"""
