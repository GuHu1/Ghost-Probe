...
BEV 网格：
  BEV_RANGE / BEV_RES 现在从 common/bev_config.py 读取（项目唯一的 BEV
  网格定义源），不再在本文件里单独写死 0.5m/200×200。要改整个项目的
  BEV 格子大小，去改 common/bev_config.py 里的 BEV_RESOLUTION_M 一个数字。
"""
...
# BEV_RANGE = (x0,y0,x1,y1) m ; BEV_RES = m/pixel  — 见文件头，来自 common/bev_config.py
...
def _clip_line(x1,y1,x2,y2,W,H):
    ...
    while True:
        ...
        if c_out&BOTTOM:
            x=x1+(x2-x1)*(H-y1)/(y2-y1) if y2!=y1 else x1; y=float(H)
        elif c_out&TOP:
            x=x1+(x2-x1)*(0-y1)/(y2-y1) if y2!=y1 else x1; y=0.
        elif c_out&RIGHT:
            y=y1+(y2-y1)*(W-x1)/(x2-x1) if x2!=x1 else y1; x=float(W)
        else:
            y=y1+(y2-y1)*(0-x1)/(x2-x1) if x2!=x1 else y1; x=0.
...
def build_all_pa_labels(nusc):
    ...
    for inst in tqdm(nusc.instance, desc="扫描 instances"):
        ...
        for i in range(1, len(chain)):
            ...
            if cur['visibility'] < VIS_EMERGED: continue
            if prev['visibility'] > VIS_OCCLUDED: continue
            ...
            for j in range(i-1, -1, -1):
                ...
                if past['visibility'] > VIS_OCCLUDED: continue  # 跳过不中断
...
def _gaussian_radius(h,w,min_overlap=0.7):
    def r(a,b,c):
        d=b*b-4*a*c; return (b-np.sqrt(max(d,0)))/(2*a) if a else 0
...
def _draw_gaussian(hm,cr,cc,radius):
    ...
    hm[r0:r1,c0:c1]=np.maximum(hm[r0:r1,c0:c1],np.exp(-(rr**2+cc_**2)/(2*s**2)))
    return hm
...
def compute_bev_gt(nusc, sample_token, pa_labels):
    ...
    for lbl in pa_labels:
        ...
        if not np.isnan(vel).any():
            vl=(G2L[:3,:3]@vel)[:2]
            vm[0,row,col]=float(vl[0]); vm[1,row,col]=float(vl[1])
...
def _draw_3d_box_on_ax(ax,lbl,ep,cs,K,W,H,color,lw=0.7):
    ...
    if(np.all(x2d<-margin) or np.all(x2d>W+margin) or
       np.all(y2d<-margin) or np.all(y2d>H+margin)): return False
...
def visualize_multicam(nusc, sample_token, frame_labels, save_path):
    ...
    for ax_i,cam_ch in enumerate(['CAM_FRONT','CAM_FRONT_LEFT','CAM_FRONT_RIGHT']):
        ...
        ax.set_title(f'{cam_ch}  |  PA目标: {drawn}  细线=先验尺寸  黄箭头=V_emerge',
                     color='#aaaaff',fontsize=9); ax.axis('off')
...
def visualize_bev(nusc, sample_token, frame_labels, frame_data, save_path):
    ...
    ax.axhline(0,color='white',lw=0.4,alpha=0.2); ax.axvline(0,color='white',lw=0.4,alpha=0.2)
...
def copy_sensor_files(nusc, sample_token, base: Path):
    sam=nusc.get('sample',sample_token)
    ...
    tok_prev=sd_lidar['prev']; count=0
    while tok_prev and count<N_SWEEPS:
        sd_sw=nusc.get('sample_data',tok_prev)
        src=Path(DATAROOT)/sd_sw['filename']; dst=base/sd_sw['filename']
        dst.parent.mkdir(parents=True,exist_ok=True)
        if not dst.exists() and src.exists(): shutil.copy2(src,dst)
        tok_prev=sd_sw['prev']; count+=1

def copy_maps_and_meta(base: Path):
...
def main():
    ...
    parser.add_argument('--vis_n',type=int,default=5,help='-1=全部正样本')
    ...
    for st,labels in tqdm(pos_labels.items(),desc="pos BEV"):
        ...
        pos_full[st]={'token':st,'pa_instances':labels,'heatmap':hm,'velocity_map':vm,
                      'weight_map':wm,'exist_map':ex,'n_pos':sum(1 for l in labels if l['is_positive']),'has_pa':True}
    for st,labels in neg_labels.items():
        neg_full[st]={'token':st,'pa_instances':labels,
    ...
    for sample in nusc.sample:
        ...
        if st in pos_full: all_full[st]=pos_full[st]
            ...
            all_full[st]={'token':st,'pa_instances':[],
                          'heatmap':np.zeros((H_BEV,W_BEV),np.float32),
                          'velocity_map':np.zeros((2,H_BEV,W_BEV),np.float32),
                          'weight_map':np.ones((H_BEV,W_BEV),np.float32),
    ...
    # 统计
    type_c={0:0,1:0,2:0}; k_dist={}
    for labels in pos_labels.values():
        for l in labels:
            type_c[l['pa_type']]+=1; k_dist[l['k']]=k_dist.get(l['k'],0)+1
    report=f"""
{'='*55}
  {VERSION} PA 标注统计
{'='*55}
